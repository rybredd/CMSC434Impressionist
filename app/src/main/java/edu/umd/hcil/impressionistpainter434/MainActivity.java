...
public class MainActivity extends AppCompatActivity implements OnMenuItemClickListener {
    ...
    public boolean onMenuItemClick(MenuItem item) {
        switch (item.getItemId()) {
            ...
            case R.id.menuTriangle:
                ...
                return true;
            case R.id.menuHexagon:
                Toast.makeText(this, "Hexagon Brush", Toast.LENGTH_SHORT).show();
                _impressionistView.setBrushType(BrushType.Hexagon);
                return true;
        }
        return false;
    }
    ...
    /* Activity method for handling sharing an image */
    public void onButtonClickShareImage(View v) {
        ImpressionistView iv = (ImpressionistView) findViewById(R.id.viewImpressionist);

        String imagePath = iv.savePainting(getApplicationContext());
        System.out.println(imagePath);

        File image = new File(imagePath);
        Uri screenshot = Uri.fromFile(image);

        Intent shareIntent = new Intent(Intent.ACTION_SEND);
        shareIntent.putExtra(android.content.Intent.EXTRA_SUBJECT,"CMSC434 Impressionist Painting");
        shareIntent.putExtra(android.content.Intent.EXTRA_TEXT, "Check out this cool impressionist painting I did!");
        shareIntent.setType("image/*");
        shareIntent.putExtra(Intent.EXTRA_STREAM, screenshot);
        startActivity(Intent.createChooser(shareIntent, "Send mail..."));
    }

    /* Activity method for handling saving an image */
    public void onButtonClickSaveImage(View v) {
        ImpressionistView iv = (ImpressionistView) findViewById(R.id.viewImpressionist);
        if (iv.savePainting(getApplicationContext()) == "ERROR") {
            Toast.makeText(getApplicationContext(), "Error! Could not save your drawing!", Toast.LENGTH_LONG).show();
        } else {
            Toast.makeText(getApplicationContext(), "Saved Drawing to Gallery!", Toast.LENGTH_LONG).show();
        }
    }
